...
- Report the last actual stage event when collection/baseline stages repeat; a native-collection failure must not be shown as stopping at an earlier generated baseline. Added a regression for stage and timestamp.
...
- `npm test`: **23 tests passed**, including a regression test for delayed artifact responses after retention.
- `npm run build`: passed natively and in the documented Node 22 Docker build with Vite 8.3.0, Vitest 5.0.1 and React Router 7.18.4.
